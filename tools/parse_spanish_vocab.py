...
def main():
    ...
    # _emit_js_bank(merged) — DISABLED 2026-07-22. subjects/spanish/vocab-bank.js
    # (the servable file Vocab Lab actually loads) is now generated by
    # tools/build_vocab_bank_by_topic.js from the owner-supplied topic-mapped
    # wordbank instead. This script's OWN output (OUT_PATH, above) is kept
    # around as the PDF-verified reference used for rank backfill by that
    # script — do NOT re-enable this call, it would silently overwrite the
    # topic-mapped file with the old (topic-less) schema.
...
def _emit_js_bank(entries):
    ...
    NOT CALLED as of 2026-07-22 — see the comment at its call site above."""
