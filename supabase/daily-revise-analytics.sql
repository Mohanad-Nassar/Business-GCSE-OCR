...
-- ── Usage: per-student activity volume ──
-- p_days windows attempts/days_revised (null = all time); last_revised_at
-- is always all-time so "days since last revised" stays meaningful even in
-- a 7-day view.
create or replace function get_class_dr_usage(p_class_id uuid, p_days int default null)
returns table (student_id uuid, attempts int, days_revised int, last_revised_at timestamptz)
...
begin
    ...
    select cs.student_id,
           count(pe.id) filter (where p_days is null
                                   or pe.answered_at >= now() - make_interval(days => p_days))::int,
...
$$;
grant execute on function get_class_dr_usage(uuid, int) to authenticated;
...
create or replace function get_class_dr_overview(p_class_id uuid)
...
$$;
grant execute on function get_class_dr_overview(uuid) to authenticated;
...
-- all time). option_counts maps each distinct submitted value (MCQ option
-- index as text, TF 'true'/'false') to how many times it was chosen; FIB
-- answers are multi-blank objects, so option_counts is null for them and
-- the client shows a correct/incorrect split instead. answer_key exposure
-- here is teacher-only (the guard above) — the same content teachers
-- already see in the task builder's mark schemes.
create or replace function get_class_dr_questions(p_class_id uuid, p_days int default 90)
...
$$;
grant execute on function get_class_dr_questions(uuid, int) to authenticated;
...
create or replace function get_class_dr_matrix(p_class_id uuid) returns jsonb
language plpgsql security definer stable set search_path = public as $$
declare
    v_pages jsonb;
    v_cells jsonb;
...
$$;
grant execute on function get_class_dr_matrix(uuid) to authenticated;
